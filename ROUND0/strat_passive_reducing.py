...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "TOMATOES" and last_bid is not None and last_ask is not None:
                ...
                if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
                    ...
                    if best_bid - last_bid > 3 and position > 5:
                        ...
                        print(f"[TOMATOES] SELL Reducing position by {max(-best_bid_amount, -position)} at {best_bid}")

                    orders.append(Order(product, best_bid + 1, 10))
                    print(f"[TOMATOES] Placing BUY order at {best_bid + 1}")

                    if last_ask - best_ask > 3 and position < -5:
                        orders.append(Order(product, best_ask, min(-best_ask_amount, -position)))
                        print(f"[TOMATOES] BUY Reducing position by {min(-best_ask_amount, -position)} at {best_ask}")

                    orders.append(Order(product, best_ask - 1, -10))
                    print(f"[TOMATOES] Placing SELL order at {best_ask - 1}")
